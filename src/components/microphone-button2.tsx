...
export default function MicrophoneButton() {
  ...
  const conversation = useConversation({
    onConnect: (connectionData: any) => {
      console.log("Connected with details:", {
        status: "connected",
    ...
    onDisconnect: () => {
      console.log("Disconnected", {
        ...
        lastState: conversation // Возможно, здесь есть полезные данные
      });
      setAudioStream(null);
    ...
    onMessage: (message: any) => {
      console.log("Message: ", {
        ...
        timestamp: new Date().toISOString(),
        rawMessage: message // Полный объект сообщения
      });
    ...
    onError: (error: any) => console.error("Error:", {
      error: error,
      timestamp: new Date().toISOString(),
      stack: error.stack // Если доступно
    }),
  });
  ...
  return (
    <div className="flex flex-col items-center gap-4" style={{ width: "100%" }}>
      {conversation.status === "connected" && ( true
        // <AudioVisualizer
